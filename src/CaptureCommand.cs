...
using NDesk.Options;
using ZeroInstall.Publish.Capture.Cli.Properties;

namespace ZeroInstall.Publish.Capture.Cli;
...
internal class CaptureCommand
{
    ...
    private OptionSet BuildOptions()
    {
        ...
        options.Add("h|help|?", () => Resources.OptionHelp, _ =>
        {
            ...
            Console.WriteLine(@"0capture start SNAPSHOT-FILE [--force]");
            Console.WriteLine(@"0capture finish SNAPSHOT-FILE FEED-FILE [--force]");
            Console.WriteLine();
    ...
    public void Execute()
    {
        switch (_additionalArgs)
        {
            ...
            case ["finish", var snapshotFile, var feedFile]:
                ...
                break;
            default:
                throw new OptionException(string.Format(Resources.MissingArguments, "0capture --help"), "");
        }
    }
